...
from auc_biases.utils import NumpyEncoder
...
class Job:
    ...
    DONE = 'Done'
    RUNNING = 'Running'
    EXT_ARGS = [re.findall(r"--([a-zA_Z0-9_]+)", i)[0] for i in (Path(__file__).parent.resolve()/'auc_biases'/'train.py').open('r')
    ...
    def __init__(self, train_args, sweep_output_root, slurm_pre, script_name, no_output_dir=False, running_jobs_list=[]):
        ...
        if not no_output_dir:
            ...
            self.train_args['output_dir'] = self.output_folder

        command = ['python -m', 'auc_biases.'+ script_name]
